...
export function useCFR(childId) {
  const progress = useLiveQuery(
    () => {
      if (!childId) return [];
      return db.progress.where('child_id').equals(parseInt(childId)).toArray();
    },
    [childId]
  );
    ...
    try {
      ...
      const quick = scored
        ...
        .sort((a, b) => {
          ...
          return (a.level || 1) - (b.level || 1) || b.priority - a.priority;
        });
      ...
      console.error('CFR Calculation failed:', err);
      return [];
    }
  }, [progress]);
